...
function makeMockRuntime(): ComplianceRuntime {
	...
	const mock = { acceptVerdict: () => Promise.resolve() } as unknown as ComplianceRuntime;
	return mock;
}
...
describe("createComplianceAdvisorHook", () => {
	...
	it("the compliance_verdict tool calls runtime.acceptVerdict on valid submission", async () => {
		...
		const acceptSpy = spyOn(runtime, "acceptVerdict").mockImplementation(() => Promise.resolve());
		...
		const _toolResult = await tool.handler(verdict);
	...
	it("does NOT call acceptVerdict when the verdict attempt does not match the envelope", async () => {
		...
		await expect(tool.handler(verdict)).rejects.toThrow();
