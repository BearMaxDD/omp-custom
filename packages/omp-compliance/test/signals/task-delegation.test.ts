...
describe("task-delegation 证据采集 — 只接受有真实结果的官方 task 委派", () => {
	it("成功完成的 task 委派记录为 completed 状态", () => {
		...
		collector.recordCall(
			taskCall({ agent: "implementer", assignment: "实现 fixture" }, "t1"),
		);
		collector.recordResult(
			taskResult({ agentId: "agent-42", exitCode: 0, output: "参照 src/a.ts" }, "t1"),
		);
	...
	it("退出码非零时标记为 aborted", () => {
		...
		collector.recordResult(
			taskResult({ agentId: "agent-x", exitCode: 1, aborted: true }, "t3"),
		);
		const snap = collector.snapshot();
	...
	it("无 result 的 task call 标记为 insufficient", () => {
		const collector = new ToolEventCollector();
		collector.recordCall(
			taskCall({ agent: "runner", assignment: "测试" }, "t5-no-result"),
		);
		const snap = collector.snapshot();
		expect(snap.subagentDelegations[0].status).toBe("insufficient");
	});
	...
	it("错误 result 也标记为 insufficient", () => {
		const collector = new ToolEventCollector();
		collector.recordCall(
			taskCall({ agent: "runner", assignment: "崩溃任务" }, "t6-error"),
		);
	...
	it("提取 codebase 引用", () => {
		const collector = new ToolEventCollector();
		collector.recordCall(
			taskCall({ assignment: "修复 src/core.ts" }, "t7-refs"),
		);
		collector.recordResult(
			taskResult(
				{
